...
pub static mut ecx_mapt: [ecx_mapt_t; 1] = [ecx_mapt_t {
    ...
    running: 0,
    context: unsafe {
        mem::transmute::<[u8; size_of::<ecx_contextt>()], ecx_contextt>(
            [0u8; size_of::<ecx_contextt>()],
        )
    },
    slave: 0,
}; 1];
...
unsafe fn ecx_lookup_prev_sii(context: &mut ecx_contextt, slave: u16) -> libc::c_int {
    ...
    if slave as libc::c_int > 1i32 && context.slavelist.len() > 0 {
        i = 1i32;
        while (context.slavelist[i as usize].eep_man != context.slavelist[slave as usize].eep_man
            || context.slavelist[i as usize].eep_id != context.slavelist[slave as usize].eep_id
            || context.slavelist[i as usize].eep_rev != context.slavelist[slave as usize].eep_rev)
            && i < slave as libc::c_int
        {
            i += 1
        }
        if i < slave as libc::c_int {
            context.slavelist[slave as usize].CoEdetails = context.slavelist[i as usize].CoEdetails;
            context.slavelist[slave as usize].FoEdetails = context.slavelist[i as usize].FoEdetails;
            context.slavelist[slave as usize].EoEdetails = context.slavelist[i as usize].EoEdetails;
            context.slavelist[slave as usize].SoEdetails = context.slavelist[i as usize].SoEdetails;
            ...
            context.slavelist[0].Ebuscurrent += context.slavelist[slave as usize].Ebuscurrent;
            memcpy(
                context.slavelist[slave as usize].name.as_mut_ptr() as *mut libc::c_void,
                context.slavelist[i as usize].name.as_ptr() as *const libc::c_void,
                (40i32 + 1i32) as usize,
            );
            nSM = 0i32;
            while nSM < 8i32 {
            ...
            context.slavelist[slave as usize].FMMU0func = context.slavelist[i as usize].FMMU0func;
            context.slavelist[slave as usize].FMMU1func = context.slavelist[i as usize].FMMU1func;
            context.slavelist[slave as usize].FMMU2func = context.slavelist[i as usize].FMMU2func;
            context.slavelist[slave as usize].FMMU3func = context.slavelist[i as usize].FMMU3func;
            return 1i32;
        }
    }
    return 0i32;
        ...
        while slave as libc::c_int <= context.slavelist.len() as i32 {
            ...
            if cindex == 0 && ecx_lookup_prev_sii(context, slave) == 0 {
                ...
                if nSM > 0i32 {
                    ...
                    context.slavelist[slave as usize].SM[0usize].SMflags = ((*context.eepSM).Creg
                        as libc::c_int
                        + (((*context.eepSM).Activate as libc::c_int) << 16i32))
                        as u32;
                    SMc = 1u8;
                    while (SMc as libc::c_int) < 8i32
                        && ecx_siiSMnext(context, slave, context.eepSM, SMc as u16) as libc::c_int
                            != 0
                    {
...
pub unsafe fn ecx_config_map_group(
    ...
    if context.slavelist.len() > 0 && (group as libc::c_int) < context.maxgroup {
        ...
        *fresh15 =
            (pIOmap as *mut u8).offset((*context.grouplist.offset(group as isize)).Obytes as isize);
