...
pub unsafe extern "C" fn osal_usleep(mut usec: uint32) -> libc::c_int {
    ...
    ts.tv_sec =
        usec.wrapping_div(1000000 as libc::c_int as libc::c_uint) as __time_t;
    ts.tv_nsec =
        usec.wrapping_rem(1000000 as libc::c_int as
                              libc::c_uint).wrapping_mul(1000 as libc::c_int
                                                             as libc::c_uint)
...
pub unsafe extern "C" fn osal_current_time() -> ec_timet {
    let mut current_time: timespec = timespec{tv_sec: 0, tv_nsec: 0,};
    let mut return_value: ec_timet = ec_timet{sec: 0, usec: 0,};
    clock_gettime(0 as libc::c_int, &mut current_time);
    return_value.sec = current_time.tv_sec as uint32;
    return_value.usec =
        (current_time.tv_nsec / 1000 as libc::c_int as libc::c_long) as
            uint32;
    return return_value;
}
#[no_mangle]
pub unsafe extern "C" fn osal_time_diff(mut start: *mut ec_timet,
                                        mut end: *mut ec_timet,
                                        mut diff: *mut ec_timet) {
    if (*end).usec < (*start).usec {
        (*diff).sec =
            (*end).sec.wrapping_sub((*start).sec).wrapping_sub(1 as
                                                                   libc::c_int
                                                                   as
                                                                   libc::c_uint);
        (*diff).usec =
            (*end).usec.wrapping_add(1000000 as libc::c_int as
                                         libc::c_uint).wrapping_sub((*start).usec)
    } else {
...
#[no_mangle]
pub unsafe extern "C" fn osal_timer_start(mut self_0: *mut osal_timert,
                                          mut timeout_usec: uint32) {
    let mut start_time: timeval = timeval{tv_sec: 0, tv_usec: 0,};
    let mut timeout: timeval = timeval{tv_sec: 0, tv_usec: 0,};
    let mut stop_time: timeval = timeval{tv_sec: 0, tv_usec: 0,};
    osal_getrelativetime(&mut start_time);
    timeout.tv_sec =
        timeout_usec.wrapping_div(1000000 as libc::c_int as libc::c_uint) as
            __time_t;
    timeout.tv_usec =
        timeout_usec.wrapping_rem(1000000 as libc::c_int as libc::c_uint) as
            __suseconds_t;
    stop_time.tv_sec = start_time.tv_sec + timeout.tv_sec;
    stop_time.tv_usec = start_time.tv_usec + timeout.tv_usec;
    if stop_time.tv_usec >= 1000000 as libc::c_int as libc::c_long {
        stop_time.tv_sec += 1;
        stop_time.tv_usec -= 1000000 as libc::c_int as libc::c_long
    }
    (*self_0).stop_time.sec = stop_time.tv_sec as uint32;
    (*self_0).stop_time.usec = stop_time.tv_usec as uint32;
}
#[no_mangle]
pub unsafe extern "C" fn osal_timer_is_expired(mut self_0: *mut osal_timert)
 -> boolean {
    ...
    is_not_yet_expired =
        ...
        } else { (current_time.tv_sec < stop_time.tv_sec) as libc::c_int };
    return (is_not_yet_expired == 0 as libc::c_int) as libc::c_int as boolean;
}
#[no_mangle]
...
#[no_mangle]
pub unsafe extern "C" fn osal_free(mut ptr: *mut libc::c_void) { free(ptr); }
#[no_mangle]
pub unsafe extern "C" fn osal_thread_create(mut thandle: *mut libc::c_void,
 ...
 -> libc::c_int {
    ...
    ret =
        pthread_create(threadp, &mut attr,
                       ::core::mem::transmute::<*mut libc::c_void,
                                                Option<unsafe extern "C" fn(_:
                                                                                *mut libc::c_void)
                                                           ->
                                                               *mut libc::c_void>>(func),
                       param);
    if ret < 0 as libc::c_int { return 0 as libc::c_int }
    return 1 as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn osal_thread_create_rt(mut thandle: *mut libc::c_void,
                                               mut stacksize: libc::c_int,
                                               mut func: *mut libc::c_void,
                                               mut param: *mut libc::c_void)
 -> libc::c_int {
    ...
    if ret < 0 as libc::c_int { return 0 as libc::c_int }
    memset(&mut schparam as *mut sched_param as *mut libc::c_void,
           0 as libc::c_int,
           ::core::mem::size_of::<sched_param>() as libc::c_ulong);
    schparam.sched_priority = 40 as libc::c_int;
    ret = pthread_setschedparam(*threadp, 1 as libc::c_int, &mut schparam);
    if ret < 0 as libc::c_int { return 0 as libc::c_int }
    return 1 as libc::c_int;
}
